...
function SideBar({ active, role }) {
    ...
    return (
        <div className="flex flex-col bg-white min-w-16 lg:min-w-64 h-[calc(100vh-5rem)] sticky top-[5rem] left-0 overflow-auto">
            <div className="bg-myprimary">
                {menu.map((item, index) => (
                    ...
                    >
                        {item.icon}
                        <p className={`text-lg hidden lg:flex`}>{item.title}</p>
                    </Link>
                ))}
            </div>
        </div>
